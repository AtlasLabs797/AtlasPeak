...
class AppDatabaseInstrumentedTest {
    ...
    fun seedIsIdempotentAndCreatesSingletonRows() = runTest {
        ...
        assertEquals(4, database.routineDao().countDefaultRoutines())
        assertEquals(7, database.weeklyPlanDao().getPlan().size)
        assertEquals("CARDIO", database.weeklyPlanDao().getPlan().single { it.dayOfWeek == 3 }.type)
